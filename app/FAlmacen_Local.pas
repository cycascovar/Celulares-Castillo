...
uses
  ...
  ExcelXP,  uCore, pngimage, Excel2000, TypInfo;

type
  TMes = array[1..12] of String;
// --------
type
  TFAlmacenLocal = class(TForm)
    ...
    procedure repmensualClick(Sender: TObject);
  ...
  public
    ...
    const
    _month : TMes =  ('Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre');

    procedure traerTodos();
    function totalInventario(ZQAlmacen:TZQuery): Integer;
    function StrToTMes(S : String):TMes;
  end;
...
implementation
uses
    FPrincipalEmpleados, F_Login, FR_Almacen, FR_VentaDiario;
{$R *.dfm}
...
procedure TFAlmacenLocal.btnReporteClick(Sender: TObject);
begin
//    FAlmacenLocal.Close;
    FRAlmacen.QRAlmacen.PrinterSetup;
    FRAlmacen.QRAlmacen.Print;
end;
...
//        ZQVenta.SQL.Add('SELECT COUNT(*) AS Total FROM venta_detalle WHERE sucursal="'+Fprincipal.sucursal+'" AND fecha_venta="'+DateToStr(Date())+'"');
        ZQVenta.SQL.Add('SELECT COUNT(*) AS Total FROM venta_detalle WHERE sucursal="'+getSucursal+'"');
        ZQVenta.ExecSQL;
        ZQVenta.Open;
...
//        ZQVenta.SQL.Add('SELECT SUM(precio) AS Venta FROM venta_detalle WHERE sucursal="'+Fprincipal.sucursal+'" AND fecha_venta="'+DateToStr(Date())+'"');
        ZQVenta.SQL.Add('SELECT SUM(precio) AS Venta FROM venta_detalle WHERE sucursal="'+getSucursal+'"');
        ZQVenta.ExecSQL;
        ZQVenta.Open;
         ...
         begin
             ...
             begin
                Application.ProcessMessages;
                 ...
                 Hoja.Cells.Item[i,6].WrapText := TRUE;
                 Hoja.Cells.Item[i,7] := ZQVenta.FieldByName('ICCID').AsVariant;
...
procedure TFAlmacenLocal.repmensualClick(Sender: TObject);
var
  mes : String;
  i : Integer;
begin
    i := 0;
{  if repMensual.Checked = true then
  begin
    try
      mes := InputBox('Realizar reporte por mes','Escriba el mes del que desea hacer el reporte: '+#13+'(Ejemplo: Enero, Febrero...)','');
      while i < 12 do
      begin
        if mes = _Month[i] then
        begin
            ShowMessage(_month[i]+' = '+mes);
            ShowMessage('Mes correcto!');
            break;
        end
        else
        begin
...
procedure TFAlmacenLocal.SpeedButton1Click(Sender: TObject);
begin
    FRVentaDiario.QuickRep1.PrinterSetup;
    FRVentaDiario.QuickRep1.Print;
end;

function TFAlmacenLocal.StrToTMes(S: String): TMes;
begin
//  StrToTMes := TMes(GetEnumValue(TypeInfo(TMes),S));
end;
...
begin
    ...
    totalInventario := ZQAlmacen.FieldByName('Total').AsInteger;
end;

procedure TFAlmacenLocal.traerTodos;
begin
// c
    ZQAlmacen.Close;
    ZQAlmacen.SQL.Clear();
...
begin
    ...
    total := totalInventario(ZQAlmacen);
    totalequipos.Caption := IntToStr(total);
    traerTodos();
end;
